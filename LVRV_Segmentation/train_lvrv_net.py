""" The main file to launch the training of LVRV-net """
...
from data_lvrv_train import ukbiobank_data
...
def train_lvrv_net():
    ...
    # Finetune
        ...
        print('Loading model')
        model.load_weights(filepath=os.path.join(code_path, 'LVRV_Segmentation', 'model_lvrv_net_epoch{}.h5'.format(str(current_epoch).zfill(3))) )
    ...
    #plot_model(model, to_file='model_lvrv_net.png', show_shapes=True, show_layer_names=True)
    ...
    for epoch in range(new_start_epoch + 1, new_start_epoch + epochs + 1):
        ...
        if (epoch % 5 == 0):
            model.save_weights(os.path.join(code_path, 'LVRV_Segmentation', 'model_lvrv_net_epoch{}.h5'.format(str(epoch).zfill(3))) )

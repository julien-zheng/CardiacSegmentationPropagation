""" A function to generate the lists of files for ROI-net training"""
...
def ukbiobank_data():
    ...
    for k in train_statistics:
        ...
        # We choose to use only the ED instant for training ROI-net
        used_instants = []
        if (ed_es_instant0 >= 0):
        ...
        for idx, t in enumerate(used_instants):
            ...
            for s in range(first_slice_idx, first_slice_idx + int(round(slices * 0.4 + 0.001))):
    ...
    for k in test_statistics:
        ...
        # We choose to use only the ED instant for testing ROI-net
        used_instants = []
        if (ed_es_instant0 >= 0):
            used_instants += [ed_es_instant0]
